(function () {
  ...
  var scheduler = {
    endOfTime: moment$1(32503672800000),
    debug: true,
    logging: true,
            ...
            switch (_context3.prev = _context3.next) {
              ...
              case 2:
                ...
                _loop = /*#__PURE__*/regenerator.mark(function _loop(i) {
                  var _c$dataset$sheetId, _c$dataset$sheetLooku, _sheets$get, _row$show, _row$hide, _row$videoUrl;

                  var c, id, lookup, sheet, url, res, rows, row;
                    ...
                    while (1) {
                      switch (_context2.prev = _context2.next) {
                        ...
                        case 5:
                          ...
                          url = "https://docs.google.com/spreadsheets/d/e/".concat(id, "/pub?output=csv"); // const res = await fetch(url)
                        ...
                        case 17:
                          sheet.value = _context2.sent;

                        case 18:
                          sheet.entries = scheduler.parseSheet(sheet.value);
                          rows = sheet.entries.filter(function (e) {
                            return e.key === lookup;
                          });
                        ...
                        case 23:
                          row = rows[0];
                          c.dataset.show = (_row$show = row.show) !== null && _row$show !== void 0 ? _row$show : '';
                          c.dataset.hide = (_row$hide = row.hide) !== null && _row$hide !== void 0 ? _row$hide : '';
                          c.dataset.videoUrl = (_row$videoUrl = row.videoUrl) !== null && _row$videoUrl !== void 0 ? _row$videoUrl : '';
                          c.dataset.sheetId = '';
    ...
    parseSheet: function parseSheet(text) {
      ...
      for (var i = 0; i < divs.length; i++) {
        ...
        var _container = {
          ...
          schedule: {
            ...
            timezone: (_div$dataset$timezone = div.dataset.timezone) !== null && _div$dataset$timezone !== void 0 ? _div$dataset$timezone : 'Africa/Johannesburg',
            // Default to Africa/Johannesburg timezone.
            next: {
              show: moment$1(0),
              hide: moment$1(scheduler.endOfTime)
            }
          }
        };
    ...
    getScheduleType: function getScheduleType(container) {
      var showDate = moment$1.tz(container.schedule.show, container.schedule.timezone);
      var hideDate = moment$1.tz(container.schedule.hide, container.schedule.timezone); // First we need to figure out what type of date strings we'll be working with.
    ...
    getNextTime: function getNextTime(container) {
      var now = moment$1.tz(container.schedule.timezone);
      ...
      switch (container.schedule.type) {
        case 'onceOff':
          ts = moment$1.tz(container.schedule.show, container.schedule.timezone); // Use default value if it's missing or invalid.

          if (!ts.isValid()) {
            ts = moment$1(0);
          }

          th = moment$1.tz(container.schedule.hide, container.schedule.timezone); // Use default value if it's missing or invalid.
    ...
    getDayDifference: function getDayDifference(day) {
      ...
      var now = moment$1.tz('Africa/Johannesburg'); // Get the index of the day in the show attribute.
    ...
    showHide: function showHide(container) {
      ...
      if (container.schedule.type === 'invalid') return; // TODO: I don't think I need to use the TZ here.
    ...
    clockCalc: function clockCalc(container) {
      ...
      var dur = moment$1.duration(endTime.diff(now)).add(clockEnd[0], 'minutes').add(clockEnd[1], 'seconds');
      var days = dur.days();
      var sDays = '';
    ...
    insertSheetLookup: function insertSheetLookup(container) {
      ...
      var url = "https://docs.google.com/spreadsheets/d/e/".concat(sheetId, "/pub?output=csv");
      var p = fetch(url).then(function (res) {
        return res.text();
      }).then(function (text) {
        var _f$0$videoUrl, _f$;

        var entries = scheduler.parseSheet(text);
        var f = entries.filter(function (e) {
          return e.key === sheetLookup;
        });
        if (f.length === 0) return Promise.reject();
        var src = (_f$0$videoUrl = (_f$ = f[0]) === null || _f$ === void 0 ? void 0 : _f$.videoUrl) !== null && _f$0$videoUrl !== void 0 ? _f$0$videoUrl : '';
...
//# sourceMappingURL=ts-schedule.browser.0.7.1.js.map
