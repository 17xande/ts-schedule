  ...
  var scheduler = {
        ...
        return regeneratorRuntime.wrap(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                if (scheduler.debug) scheduler.logging = true; // stop previous timeLoop, if there is one running

                scheduler.stop();
                sheetContainers = scheduler.getSheetContainers(scheduler.selector);
                _context.next = 5;
                return scheduler.updateSheetContainers(sheetContainers);

              case 5:
                scheduler.containers = scheduler.getContainers(scheduler.selector);
                scheduler.containers.forEach(function (container) {
                  return scheduler.showHide(container);
                });
    ...
    stop: function stop() {
      if (scheduler.timerID != 0) {
        if (scheduler.logging) console.log("stopping timeLoop: ".concat(scheduler.timerID));
    ...
    getSheetContainers: function getSheetContainers(selector) {
      var els = document.querySelectorAll(selector);
      var sheetContainers = Array.from(els).map(function (e) {
        return e;
      }).filter(function (d) {
        var _d$dataset$sheetLooku;

        var sheetLookup = (_d$dataset$sheetLooku = d.dataset.sheetLookup) !== null && _d$dataset$sheetLooku !== void 0 ? _d$dataset$sheetLooku : '';
    ...
    updateSheetContainers: function updateSheetContainers(sheetContainers) {
      return _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2() {
        var sheets, _loop, i, _ret;
        ...
        return regeneratorRuntime.wrap(function _callee2$(_context3) {
          while (1) {
            switch (_context3.prev = _context3.next) {
              ...
              case 2:
                sheets = new Map(); // Loop through the containers and create a map of each sheet ID
                // and each lookup address for each sheet.

                _loop = /*#__PURE__*/regeneratorRuntime.mark(function _loop(i) {
                  var _c$dataset$sheetId, _c$dataset$sheetLooku, _sheets$get, _row$gsx$show$$t, _row$gsx$show, _row$gsx$hide$$t, _row$gsx$hide, _row$gsx$videoUrl$$t, _row$gsx$videoUrl;

                  var c, id, lookup, sheet, url, res, json, entries, rows, row;
                  return regeneratorRuntime.wrap(function _loop$(_context2) {
                    while (1) {
                      switch (_context2.prev = _context2.next) {
                        case 0:
                          c = sheetContainers[i];
                          id = (_c$dataset$sheetId = c.dataset.sheetId) !== null && _c$dataset$sheetId !== void 0 ? _c$dataset$sheetId : '';
                          lookup = (_c$dataset$sheetLooku = c.dataset.sheetLookup) !== null && _c$dataset$sheetLooku !== void 0 ? _c$dataset$sheetLooku : '';
                        ...
                        case 5:
                          sheet = (_sheets$get = sheets.get(id)) !== null && _sheets$get !== void 0 ? _sheets$get : {};
                          sheet.lookup.push(lookup); // sheets = sheets.set(id, o)
                          ...
                          url = "https://spreadsheets.google.com/feeds/list/".concat(id, "/od6/public/values?alt=json");
                          _context2.next = 12;
                          return fetch(url);
                        ...
                        case 17:
                          entries = sheet.value.feed.entry;
                          rows = entries.filter(function (e) {
                            var _e$gsx$key;

                            return ((_e$gsx$key = e.gsx$key) === null || _e$gsx$key === void 0 ? void 0 : _e$gsx$key.$t) === lookup;
                          });
                          ...
                          console.error("key ".concat(lookup, " not found in sheet."));
                          return _context2.abrupt("return", {
                            v: Promise.reject()
                          });
                        ...
                        case 22:
                          row = rows[0];
                          c.dataset.show = (_row$gsx$show$$t = (_row$gsx$show = row.gsx$show) === null || _row$gsx$show === void 0 ? void 0 : _row$gsx$show.$t) !== null && _row$gsx$show$$t !== void 0 ? _row$gsx$show$$t : '';
                          c.dataset.hide = (_row$gsx$hide$$t = (_row$gsx$hide = row.gsx$hide) === null || _row$gsx$hide === void 0 ? void 0 : _row$gsx$hide.$t) !== null && _row$gsx$hide$$t !== void 0 ? _row$gsx$hide$$t : '';
                          c.dataset.videoUrl = (_row$gsx$videoUrl$$t = (_row$gsx$videoUrl = row.gsx$videoUrl) === null || _row$gsx$videoUrl === void 0 ? void 0 : _row$gsx$videoUrl.$t) !== null && _row$gsx$videoUrl$$t !== void 0 ? _row$gsx$videoUrl$$t : '';
                          c.dataset.sheetId = '';
    ...
    getContainers: function getContainers(selector) {
      var divs = document.querySelectorAll(selector); // If targeting > ES5:
      ...
      for (var i = 0; i < divs.length; i++) {
        ...
        var _container = {
          ...
          schedule: {
            ...
            hide: (_div$dataset$hide = div.dataset.hide) !== null && _div$dataset$hide !== void 0 ? _div$dataset$hide : '',
            showHide: (_div$dataset$showHide = div.dataset.showHide) !== null && _div$dataset$showHide !== void 0 ? _div$dataset$showHide : '',
            timezone: (_div$dataset$timezone = div.dataset.timezone) !== null && _div$dataset$timezone !== void 0 ? _div$dataset$timezone : '+02:00',
            // Default to +02:00 timezone.
            next: {
              show: moment(0),
              hide: moment(scheduler.endOfTime)
            }
          }
        };
        _container.schedule.type = scheduler.getScheduleType(_container);
        var sh = scheduler.getNextTime(_container);
        _container.schedule.next = sh;
        containers.push(_container);
      }

      if (scheduler.logging) console.log("containers found: ".concat(containers.length, "."));
      return containers;
    },
    // getScheduleType returns the schedule type based on the other properties of the schedule.
    getScheduleType: function getScheduleType(container) {
      var showDate = moment(container.schedule.show);
      var hideDate = moment(container.schedule.hide); // First we need to figure out what type of date strings we'll be working with.
    ...
    getNextTime: function getNextTime(container) {
      var now = moment();
      var showHide = container.schedule.showHide.split('|');
      var ts = now.clone();
      var th = now.clone();

      switch (container.schedule.type) {
        case 'onceOff':
          ts = moment(container.schedule.show); // Use default value if it's missing or invalid.

          if (!ts.isValid()) {
            ts = moment(0);
          }

          th = moment(container.schedule.hide); // Use default value if it's missing or invalid.
        ...
        case 'recurringSingle':
          var arrS = container.schedule.show.split('|');
          var arrH = container.schedule.hide.split('|');
          var timeS = arrS[1].split(':');
          var timeH = arrH[1].split(':'); // Find how many days till the next show day that was specified.

          var daysAdded = this.getDayDifference(arrS[0]); // Add those days to the show date.
          // ts.add(daysAdded, 'days')
          // Set the time of the show date.

          ts.hours(parseInt(timeS[0], 10));
          ts.minutes(parseInt(timeS[1], 10));
          ts.seconds(parseInt(timeS[2], 10)); // Do the same for the hide day.

          daysAdded = this.getDayDifference(arrH[0]); // Add those days to the show date.

          th.add(daysAdded, 'days'); // Set the time of the show date.

          th.hours(parseInt(timeH[0], 10));
          th.minutes(parseInt(timeH[1], 10));
          th.seconds(parseInt(timeH[2], 10)); // If it's today, but past the hide time, add a week to the moment.
        ...
        case 'recurringMultipleRange':
          var daysStr = showHide[0].split(',');
          var timesStr = showHide[1].split(',');
          var dist = daysStr.map(function (d) {
            return scheduler.getDayDifference(d);
          }).sort();
          ...
          if (dist[0] !== 0) {
            ...
            ts.add(dist[0], 'days');
            th.add(dist[0], 'days');
            var timesArr = timesStr[0].split('-');
            var time = timesArr[0].split(':'); // Set the time to the first time of the day.

            ts.hour(parseInt(time[0], 10));
            ts.minute(parseInt(time[1], 10));
            ts.second(0); // repeat for the hide time.

            time = timesArr[1].split(':'); // Set the time to the first time of the day.
            ...
            for (var i = 0; i < timesStr.length; i++) {
              var ti = timesStr[i].split('-')[1].split(':');
              var d = now.clone();
              d.hour(parseInt(ti[0], 10));
              d.minute(parseInt(ti[1], 10));

              if (d.isAfter(now)) {
                var tih = timesStr[i].split('-')[0].split(':');
              ...
              if (i === timesStr.length - 1) {
                ...
                ts.add(dist[1], 'days');
                th.add(dist[1], 'days');
                var times = timesStr[0].split('-'); // Set the time to the first time of the day.
    ...
    getDayDifference: function getDayDifference(day) {
      var now = moment(); // Get the index of the day in the show attribute.

      var dayIndex = scheduler.days.indexOf(day);
      ...
      var daysAdded = (dayIndex + 7 - now.day()) % 7;
      return daysAdded;
    },
    ...
    showHide: function showHide(container) {
      ...
      container.div.classList.add('hidden'); // Get the next show and hide times.

      var sche = scheduler.getNextTime(container);
      container.schedule.next = {
        show: sche.show,
        hide: sche.hide
      };
    ...
    // be shown or hidden.
    timeLoop: function timeLoop(containers) {
      var that = this;
    ...
    insertVideo: function insertVideo(container) {
      ...
      if (videoUrl) {
        template = "<div class=\"containerVideo\"><iframe src=\"".concat(videoUrl, "\" scrolling=\"no\" allowfullscreen=\"true\" allow=\"fullscreen\" frameborder=\"0\"></iframe></div>");
        container.insertAdjacentHTML('afterbegin', template);
      } else if (embedID) {
        var _window$la1InitWebPla, _window;

        template = "<div id=\"resi-video-player\" class=\"containerVideo\" data-embed-id=\"".concat(embedID, "\"></div>");
        container.insertAdjacentHTML('afterbegin', template);
        scheduler.insertScript('https://control.resi.io/webplayer/loader.min.js'); // @ts-ignore

        (_window$la1InitWebPla = (_window = window).la1InitWebPlayer) === null || _window$la1InitWebPla === void 0 ? void 0 : _window$la1InitWebPla.call(_window);
      } else {
    ...
    insertChat: function insertChat(container) {
      if (container.querySelector('.containerChat') != null) {
        // chat already there, do nothing.
        return;
      }

      var chatUrl = container.dataset.chatUrl;
      var chatPublisher = container.dataset.chatPublisher;
      var chatRoom = container.dataset.chatRoom;
      var template = '';
    ...
    insertSheetLookup: function insertSheetLookup(container) {
      if (container.querySelector('.containerVideo') != null) {
        return Promise.resolve();
      }

      var sheetId = container.dataset.sheetId;
      var sheetLookup = container.dataset.sheetLookup;
      var url = "https://spreadsheets.google.com/feeds/list/".concat(sheetId, "/od6/public/values?alt=json"); // const res = await fetch(url)
      ...
      var p = fetch(url).then(function (res) {
        ...
        var _f$0$gsx$link$$t, _f$, _f$$gsx$link;

        var entries = data.feed.entry;
        var f = entries.filter(function (e) {
          return e.gsx$key.$t === sheetLookup;
        });
        if (f.length === 0) return Promise.reject();
        var src = (_f$0$gsx$link$$t = (_f$ = f[0]) === null || _f$ === void 0 ? void 0 : (_f$$gsx$link = _f$.gsx$link) === null || _f$$gsx$link === void 0 ? void 0 : _f$$gsx$link.$t) !== null && _f$0$gsx$link$$t !== void 0 ? _f$0$gsx$link$$t : '';
        if (src === '') return Promise.reject();
        var template = "<div class=\"containerVideo\"><iframe src=".concat(src, "></iframe></div>");
        container.insertAdjacentHTML('afterbegin', template);
        return src;
      });
      return p;
    ...
    insertScript: function insertScript(src) {
      // skip if script is already inserted.
      if (document.querySelector("script[src=\"".concat(src, "\"]"))) return;
...
//# sourceMappingURL=ts-schedule.browser.0.5.1.js.map
